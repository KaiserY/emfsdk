...
impl EmfPlusObjectRecordData {
    ...
    pub fn parse_object_data(&self) -> Result<EmfPlusObjectData> {
        ...
        match self.object_type() {
            ...
            Some(EmfPlusObjectType::Font) if self.object_data.len() >= 24 => Ok(
                EmfPlusObjectData::Font(read_emf_plus_font_object(&mut reader, data_len)?),
            ),
            ...
            Some(EmfPlusObjectType::ImageAttributes) if self.object_data.len() == 24 => {
                Ok(EmfPlusObjectData::ImageAttributes(
                    EmfPlusImageAttributesObject::read_from(&mut reader)?,
                ))
            }
            Some(EmfPlusObjectType::Path) if self.object_data.len() >= 12 => Ok(
                EmfPlusObjectData::Path(read_emf_plus_path_object(&mut reader, data_len)?),
            ),
            Some(EmfPlusObjectType::Pen) if self.object_data.len() >= 8 => Ok(
                EmfPlusObjectData::Pen(read_emf_plus_pen_object(&mut reader, data_len)?),
            ),
            ...
            Some(EmfPlusObjectType::StringFormat) if self.object_data.len() >= 60 => {
                Ok(EmfPlusObjectData::StringFormat(
                    read_emf_plus_string_format_object(&mut reader, data_len)?,
                ))
            }
    ...
    pub fn parse_cap_data(&self) -> Result<EmfPlusCustomLineCapData> {
        ...
        match self.cap_data_type() {
            ...
            Some(EmfPlusCustomLineCapDataType::Default)
                if self.custom_line_cap_data.len() >= 48 =>
            {
...
impl EmfPlusPenOptionalData {
    ...
    pub fn validate_for_flags(&self, flags: EmfPlusPenDataFlags) -> Result<()> {
        ...
        if flags.contains(EmfPlusPenDataFlags::DASHED_LINE_CAP)
            && self.dashed_line_cap_type.is_none()
        {
            return Err(Error::invalid(
                0,
        ...
        if flags.contains(EmfPlusPenDataFlags::CUSTOM_START_CAP)
            && self.custom_start_cap_data.is_none()
        {
            return Err(Error::invalid(
                0,
        ...
        if !flags.contains(EmfPlusPenDataFlags::DASHED_LINE_CAP)
            && self.dashed_line_cap_type.is_some()
        {
            return Err(Error::invalid(
                0,
        ...
        if !flags.contains(EmfPlusPenDataFlags::CUSTOM_END_CAP)
            && self.custom_end_cap_data.is_some()
        {
            return Err(Error::invalid(
                0,
        ...
        if self
            ...
            .is_some_and(|_| self.dashed_line_cap_type_kind().is_none())
        {
            return Err(Error::invalid(
                0,
...
impl EmfPlusRecord {
    ...
    pub fn parse_data(&self) -> Result<EmfPlusRecordData<'_>> {
        ...
        let data = match self.record_kind() {
            ...
            Some(EmfPlusRecordType::SetTsClip) => EmfPlusRecordData::SetTsClip(
                EmfPlusSetTsClipData::read_data(self.flags, &self.data)?,
            ),
            Some(_) => {
                return Err(Error::invalid(
                    0,
        ...
        let mut record_data = Vec::with_capacity(record_data_capacity);
        {
            ...
            match data {
                ...
                EmfPlusRecordData::SerializableObject(value) => {
                    ...
                    writer
                        .write_u32(len_to_u32(value.buffer.len(), "EMF+ serializable buffer")?)?;
    ...
    pub fn read_from<R: std::io::Read + std::io::Seek>(
        ...
        let header_size = if has_total_object_size { 16 } else { 12 };
        if size < header_size {
            return Err(Error::invalid(
                offset,
    ...
    pub fn write_to<W: std::io::Write + std::io::Seek>(
        ...
        let size = payload_size.checked_add(header_size).ok_or_else(|| {
            Error::invalid(writer.position().unwrap_or(0), "EMF+ record is too large")
        })?;
...
fn read_emf_plus_boundary_path_data<R: std::io::Read + std::io::Seek>(
    ...
    Ok(EmfPlusBoundaryPathData {
        path_data: read_emf_plus_size_prefixed_path_data(
            reader,
            data_len,
...
fn read_emf_plus_custom_line_cap_default_data<R: std::io::Read + std::io::Seek>(
    ...
    let value = EmfPlusCustomLineCapDefaultData {
        ...
        optional_data: read_remaining_vec(
            reader,
            data_len,
            "EmfPlusCustomLineCapData OptionalData",
        )?,
    };
    ...
    match value.bitmap_data_type_kind() {
        Some(EmfPlusBitmapDataType::Pixel) => {
            ...
            Ok(EmfPlusBitmapPayload::Pixel(EmfPlusBitmapDataObject {
                palette,
                pixel_data: read_remaining_vec(
                    &mut reader,
                    data_len,
                    "EmfPlusBitmapData PixelData",
                )?,
            }))
        }
...
fn read_emf_plus_pen_optional_data<R: std::io::Read + std::io::Seek>(
    ...
    if flags.contains(EmfPlusPenDataFlags::DASHED_LINE) {
        optional_data.dashed_line_data = Some(EmfPlusDashedLineData {
            dashed_line_data: read_f32_array_with_u32_count(
                reader,
                data_len,
    ...
    if flags.contains(EmfPlusPenDataFlags::CUSTOM_START_CAP) {
        optional_data.custom_start_cap_data = Some(EmfPlusCustomStartCapData {
            custom_start_cap: read_size_prefixed_vec(
                reader,
                data_len,
...
fn read_emf_plus_string_format_object<R: std::io::Read + std::io::Seek>(
    ...
    let tab_stop_count =
        non_negative_count(reader.read_i32()?, "EmfPlusStringFormat TabStopCount")?;
...
fn read_emf_plus_image_effect<R: std::io::Read + std::io::Seek>(
    ...
    match EmfPlusImageEffectKind::from_guid(object_guid) {
        Some(EmfPlusImageEffectKind::Blur) => Ok(EmfPlusImageEffect::Blur(
            read_emf_plus_blur_effect(reader, data_len)?,
        )),
        Some(EmfPlusImageEffectKind::BrightnessContrast) => {
            Ok(EmfPlusImageEffect::BrightnessContrast(
        ...
        Some(EmfPlusImageEffectKind::Tint) => Ok(EmfPlusImageEffect::Tint(
            read_emf_plus_tint_effect(reader, data_len)?,
        )),
...
fn read_emf_plus_brightness_contrast_effect<R: std::io::Read + std::io::Seek>(
    ...
    let value = EmfPlusBrightnessContrastEffect {
        ...
        trailing_data: read_remaining_vec(
            reader,
            data_len,
            "BrightnessContrastEffect trailing data",
        )?,
    };
...
fn read_points<R: std::io::Read + std::io::Seek>(
    ...
    if flags.contains(EmfPlusRecordFlags::COMPRESSED) {
        let required = count.checked_mul(4).ok_or_else(|| {
            Error::invalid(0, "EMF+ compressed point payload size overflows usize")
        })?;
...
fn validate_emf_plus_record_data(
    ...
    match value {
        ...
        EmfPlusRecordData::SetAntiAliasMode(value) => {
            if value.smoothing_mode_kind().is_none() {
                return Err(Error::invalid(
                    0,
                    "EmfPlusSetAntiAliasMode SmoothingMode is invalid",
                ));
        ...
        EmfPlusRecordData::SetTextRenderingHint(value) => {
            if value.text_rendering_hint().is_none() {
                return Err(Error::invalid(
                    0,
        ...
        EmfPlusRecordData::SetInterpolationMode(value) => {
            if value.interpolation_mode().is_none() {
                return Err(Error::invalid(
                    0,
        ...
        EmfPlusRecordData::SetPixelOffsetMode(value) => {
            if value.pixel_offset_mode().is_none() {
                return Err(Error::invalid(
                    0,
        ...
        EmfPlusRecordData::SetCompositingMode(value) => {
            if value.compositing_mode().is_none() {
                return Err(Error::invalid(
                    0,
        ...
        EmfPlusRecordData::SetCompositingQuality(value) => {
            if value.compositing_quality().is_none() {
                return Err(Error::invalid(
                    0,
...
fn validate_color_curve_effect(value: &EmfPlusColorCurveEffect) -> Result<()> {
    ...
    let valid_range = match adjustment {
        ...
        EmfPlusCurveAdjustment::WhiteSaturation | EmfPlusCurveAdjustment::BlackSaturation => {
            0..=255
        }
    };
...
mod tests {
    ...
    fn emf_plus_clip_and_property_records_roundtrip() {
        ...
        assert_eq!(
            EmfPlusRecordFlags::from_bits_retain(
                EmfPlusCompositingQuality::GammaCorrected.raw() as u16
            )
            .compositing_quality(),
    ...
    fn emf_plus_point_array_drawing_records_roundtrip() {
        ...
        let record = EmfPlusRecord {
            ...
            flags: (EmfPlusRecordFlags::RELATIVE_POSITION | EmfPlusRecordFlags::COMPRESSED).bits()
                | 10,
            total_object_size: None,
        ...
        let record =
            EmfPlusRecord::from_data(&fill_closed_curve, EmfPlusRecordFlags::empty()).unwrap();
    ...
    fn emf_plus_image_string_and_driver_string_records_roundtrip() {
        ...
        invalid_record.data[4..8]
            .copy_from_slice(&(EmfPlusUnitType::Point.raw() as i32).to_le_bytes());
        assert!(invalid_record.parse_data().is_err());
        assert!(
            EmfPlusRecord::from_data(
                &EmfPlusRecordData::DrawImage(EmfPlusDrawImageData {
        ...
        let record =
            EmfPlusRecord::from_data(&draw_image_points, EmfPlusRecordFlags::empty()).unwrap();
        ...
        assert!(
            EmfPlusRecord::from_data(&invalid_draw_image_points, EmfPlusRecordFlags::empty())
                .is_err()
        );
        ...
        assert!(
            EmfPlusRecord::from_data(
                &EmfPlusRecordData::DrawDriverString(EmfPlusDrawDriverStringData {
                    font_id: 6,
                    brush: EmfPlusBrushRef::ObjectId(3),
                    driver_string_options_flags: EmfPlusDriverStringOptionsFlags::REALIZED_ADVANCE
                        .bits(),
                    glyphs: vec![65, 66],
                    glyph_positions: vec![PointF { x: 1.0, y: 2.0 }, PointF { x: 3.0, y: 4.0 }],
                    transform_matrix: None,
    ...
    fn emf_plus_object_serializable_ts_and_reserved_records_roundtrip() {
        ...
        let record =
            EmfPlusRecord::from_data(&continued_object, EmfPlusRecordFlags::empty()).unwrap();
        ...
        assert!(
            EmfPlusRecord::from_data(&invalid_total_object_size, EmfPlusRecordFlags::empty())
                .is_err()
        );
        ...
        let EmfPlusRecordData::SetTsGraphics(parsed_ts_graphics) = record.parse_data().unwrap()
        else {
        ...
        let ts_graphics_without_palette =
            EmfPlusRecordData::SetTsGraphics(EmfPlusSetTsGraphicsData {
                palette: None,
                ..ts_graphics_data.clone()
            });
        let record_without_palette =
            EmfPlusRecord::from_data(&ts_graphics_without_palette, EmfPlusRecordFlags::empty())
                .unwrap();
    ...
    fn emf_plus_path_object_point_types_roundtrip() {
        ...
        assert!(
            EmfPlusObjectData::Path(EmfPlusPathObject {
                ...
                path_point_flags: 0,
                points: EmfPlusPointData::Float(vec![PointF { x: 1.0, y: 2.0 }]),
                point_types: EmfPlusPathPointTypes::Values(vec![EmfPlusPathPointTypeValue {
                    value: 0x02,
                }]),
    ...
    fn emf_plus_graphics_object_enum_accessors_map_spec_values() {
        ...
        let font = EmfPlusFontObject {
            ...
            font_style_flags: (EmfPlusFontStyleFlags::BOLD | EmfPlusFontStyleFlags::ITALIC).bits()
                as i32,
            reserved: 0,
    ...
    fn emf_plus_image_data_bitmap_and_metafile_roundtrip() {
        ...
        let EmfPlusImageData::Bitmap(parsed_indexed_bitmap) =
            indexed_image.parse_image_data().unwrap()
        else {
    ...
    fn emf_plus_typed_object_payload_helpers_roundtrip() {
        ...
        let mut object_record = EmfPlusObjectRecordData::from_typed_data(
            3,
        ...
        let cap_object =
            EmfPlusCustomLineCapObject::from_typed_data(test_graphics_version(), &cap_data)
                .unwrap();
        assert_eq!(
    ...
    fn emf_plus_pen_payload_region_and_custom_cap_data_roundtrip() {
        ...
        assert!(invalid_nested_brush_pen.parse_pen_payload().is_err());
        assert!(
            EmfPlusObjectData::Pen(invalid_nested_brush_pen)
                .to_bytes()
                .is_err()
        );
