    ...
    while pos + EMF_RECORD_HEADER_SIZE <= data.len() {
        ...
        match record_type {
            ...
            EMR_MODIFY_WORLD_TRANSFORM if record_size >= 36 => {
                if let (Ok(transform), Ok(mode)) =
                    (read_xform(data, pos + 8), read_u32(data, pos + 32))
                {
    ...
    fn color_at(&self, x: i32, y: i32) -> EmfColor {
        match self {
            ...
            Self::PathGradient {
                ...
                let distance =
                    ((x as f32 - center.0).hypot(y as f32 - center.1) / 256.0).clamp(0.0, 1.0);
    ...
    while pos + EMF_RECORD_HEADER_SIZE <= data.len() {
        ...
        match record_type {
            ...
            EMR_POLYGON if record_size >= 28 => {
                if let Some(points) =
                    read_points_i32(data, pos + 28, read_u32(data, pos + 24)? as usize)
                {
            ...
            EMR_POLYGON16 if record_size >= 28 => {
                if let Some(points) =
                    read_points_i16(data, pos + 28, read_u32(data, pos + 24)? as usize)
                {
                    state.fill_polygon(&points);
                    state.draw_polyline(&points, true);
                }
            ...
            EMR_ARC if record_size >= 40 => {
                ...
                state.fill_arc_segment(
                    rect,
                    angle_from_emf_arc_point(
                        rect,
                        read_i32(data, pos + 24)?,
                        read_i32(data, pos + 28)?,
                    ),
            ...
            EMR_SET_DIBITS_TO_DEVICE | EMR_STRETCH_DIBITS => {
                if let Some(target) = emf_bitmap_draw_target(data, pos, record_type, record_size)? {
                    let raster =
                        decode_bitmap_record_as_raster(data, record_type, pos, record_size)?;
...
fn decode_wmf_as_raster(
    ...
    for record in &metafile.records {
        ...
        match parsed {
            ...
            WmfRecordData::Chord(value) | WmfRecordData::Pie(value) => {
                state.canvas.fill_arc_segment(
                    (
                        i32::from(value.left),
                        i32::from(value.top),
                        i32::from(value.right),
...
fn dib_to_png(
    ...
    for row in 0..height_abs {
        ...
        match bit_count {
            DIB_BIT_COUNT_24 => {
                for col in 0..width {
                    let src_pixel = &src[col * RGB_BYTES_PER_PIXEL
                        ..col * RGB_BYTES_PER_PIXEL + RGB_BYTES_PER_PIXEL];
                    let dest_pixel = &mut dest[col * RGB_BYTES_PER_PIXEL
                        ..col * RGB_BYTES_PER_PIXEL + RGB_BYTES_PER_PIXEL];
            ...
            DIB_BIT_COUNT_32 => {
                for col in 0..width {
                    let src_pixel = &src[col * BGRA_BYTES_PER_PIXEL
                        ..col * BGRA_BYTES_PER_PIXEL + BGRA_BYTES_PER_PIXEL];
                    let dest_pixel = &mut dest[col * RGB_BYTES_PER_PIXEL
                        ..col * RGB_BYTES_PER_PIXEL + RGB_BYTES_PER_PIXEL];
                    dest_pixel[0] = src_pixel[2];
                    dest_pixel[1] = src_pixel[1];
    ...
    match record {
        ...
        EmfPlusRecordData::FillRects(value) => {
            ...
            for rect in value.rects {
                ...
                state.fill_polygon_with_emf_plus_brush(
                    &rect_points(left, top, right, bottom),
                    &brush,
                );
            }
        }
        ...
        EmfPlusRecordData::TranslateWorldTransform(value) => {
            multiply_emf_plus_transform(
                translate_transform(value.data),
                value.post_multiply,
                state,
            );
        }
...
fn emf_plus_brush_ref(
    ...
    match brush {
        EmfPlusBrushRef::Color(color) => {
            Some(EmfPlusRenderBrush::Solid(emf_plus_argb_to_color(color)))
        }
...
fn emf_plus_image_object_to_rgb(
    ...
    match image.parse_image_data().map_err(|err| err.to_string())? {
        ...
        EmfPlusImageData::Metafile(metafile) => {
            let Some(raster) = decode_metafile_as_raster(&metafile.metafile_data, None)
...
fn emf_plus_pixel_bitmap_to_rgb(
    ...
    for row in 0..height {
        ...
        let dest =
            &mut rgb[row * width * RGB_BYTES_PER_PIXEL..(row + 1) * width * RGB_BYTES_PER_PIXEL];
        for col in 0..width {
            let src_pixel = &src[col * bytes_per_pixel..col * bytes_per_pixel + bytes_per_pixel];
            let dest_pixel = &mut dest
                [col * RGB_BYTES_PER_PIXEL..col * RGB_BYTES_PER_PIXEL + RGB_BYTES_PER_PIXEL];
            dest_pixel[0] = src_pixel[2];
            dest_pixel[1] = src_pixel[1];
            dest_pixel[2] = src_pixel[0];
        }
...
fn packed_dib_to_rgb(
    ...
    let dib = DeviceIndependentBitmap::from_packed_slice(data, color_usage)
        .map_err(|err| err.to_string())?;
...
mod tests {
    use super::*;
    use crate::{
        BitmapSourceBounds, DibColorUsage, EMR_EOF, EMR_HEADER, EmfMetafile, EmfRecord,
        EmfRecordData, EmrBitmapBuffer, EmrStretchDiBits, RectL, SdkEnumValue, SizeL,
    };
